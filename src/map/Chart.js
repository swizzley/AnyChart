...
anychart.mapModule.Chart.prototype.calculate = function() {
  ...
  if (this.hasInvalidationState(anychart.ConsistencyState.MAP_GEO_DATA)) {
    ...
    if (goog.isDefAndNotNull(geoData)) {
      ...
      var defaultTx = goog.isObject(mapTx) ? mapTx['default'] : (mapTx = {'default': mapTx});
      if (!defaultTx)
      ...
      goog.object.forEach(mapTx || anychart.mapModule.Chart.DEFAULT_TX, function(value, key) {
        ...
        tx_.srcCrs = anychart.enums.MapProjections.WSG84;
        tx_.curProj = anychart.mapModule.projections.getProjection(tx_.crs);
        tx_.srcProj = new anychart.mapModule.projections.Base();
  ...
  if (this.hasInvalidationState(anychart.ConsistencyState.MAP_SCALE)) {
    if (this.projectionMap) {
      ...
      if ((defaultProjection.crs != this.newCrs_ && defaultProjection.srcCrs != this.newCrs_) || this.crsAnimation_.enabled()) {
        if (!anychart.mapModule.projections.isBaseProjection(defaultProjection.srcCrs)) {
          sourceProjection = defaultProjection.srcProj;
        }
        if (changeProjection) {
          destinationProjection = anychart.mapModule.projections.getProjection(this.newCrs_);
        }
      }
      currentProjection = defaultProjection.curProj;
      ...
      if (changeProjection) {
        ...
        defaultProjection.crs = this.currentCrs_;
        defaultProjection.curProj = anychart.mapModule.projections.getProjection(defaultProjection.crs);
      }
      ...
      if ((this.crsMapAnimation && this.crsMapAnimation.isStopped()) || !this.crsMapAnimation) {
        ...
        if (isAnimate) {
          defaultProjection.curProj = new anychart.mapModule.projections.TwinProjection(
          ...
          this.crsMapAnimation.listenOnce(goog.fx.Transition.EventType.END,
              ...
              }, true, {map: this, tx: /** @type {Object} */(defaultProjection)});
          this.crsMapAnimation.play();

        } else {
          var callback = goog.bind(this.calcGeom_, {
            tx: defaultProjection,
  ...
  if (this.hasInvalidationState(anychart.ConsistencyState.MAP_MOVE)) {
    if (this.getZoomLevel() != minZoomLevel && mapLayer) {
      ...
      if (dx || dy) {
        ...
        defaultProjection = mapLayer.getSelfTransformation();
        this.scale().setOffsetFocusPoint(defaultProjection.getTranslateX(), defaultProjection.getTranslateY());
        this.updateSeriesOnZoomOrMove();
      }
...
anychart.mapModule.Chart.prototype.translateFeature = function(id, dx, dy) {
  ...
  if (feature && !this.isSvgGeoData()) {
    ...
    featureTx = current_tx == this.projectionMap['default'] ? (this.projectionMap[id] = {}) : current_tx;
...
anychart.mapModule.Chart.prototype.featureScaleFactor = function(id, opt_ratio) {
  ...
  if (goog.isDef(opt_ratio)) {
    if (feature && !this.isSvgGeoData()) {
      ...
      var defaultScale = this.projectionMap['default'].scale || anychart.mapModule.Chart.DEFAULT_TX['default']['scale'];
  ...
  return feature ? featureTx.scale || this.projectionMap['default'].scale || 1 : NaN;
};
...
anychart.mapModule.Chart.prototype.featureCrs_ = function(feature, opt_crs) {
  ...
  if (!goog.isDef(opt_crs)) {
    return goog.isDef(current_tx.crs) ? current_tx.crs : this.projectionMap['default'].crs;
    ...
    var featureTx = current_tx == this.projectionMap['default'] ? (this.projectionMap[id] = {}) : current_tx;
    var old_crs = featureTx.crs || this.projectionMap['default'].crs || anychart.mapModule.Chart.DEFAULT_TX['default']['crs'];
    var new_crs = opt_crs;
    ...
    var xoffset = featureTx.xoffset || 0;
    var yoffset = featureTx.yoffset || 0;
    var featureScale = featureTx.scale || this.projectionMap['default'].scale || anychart.mapModule.Chart.DEFAULT_TX['default']['scale'];
...
anychart.mapModule.Chart.prototype.toGeoJSON = function() {
  return anychart.mapModule.utils.GeoJSONParser.getInstance().exportToGeoJSON(this.internalSourceGeoData, this.projectionMap);
};
